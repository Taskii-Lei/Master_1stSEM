...
void* handle_https_request(void *arg) {
	...
	if (!range_flag) {
		...
		//==================================================
		// .将实体内容打印出文件
		FILE *fp_200 = fopen("200_ok.txt","w+")
		int l = 0;
		while(response_buff[l] != '\0')
		{
			fprintf(fp_200, "%c", response_buff[l]);
			l++;
		}
		fprintf(fp_200, "0\r\n\r\n");
		fclose(fp_200)
		...
		if (range_end == -1) {
			...
			//==================================================
			// .将实体内容打印出文件
			FILE *fp_200 = fopen("206_range_n.txt","w+")
			int l = 0;
			while(response_buff[l] != '\0')
			{
				fprintf(fp_200, "%c", response_buff[l]);
			...
			//==================================================
			// .将实体内容打印出文件
			FILE *fp_200 = fopen("206_range_nm.txt","w+")
			int l = 0;
			while(response_buff[l] != '\0')
			{
				fprintf(fp_200, "%c", response_buff[l]);
...
void* handle_http_request(void *sockfd) {
	...
	//==================================================
	// .将实体内容打印出文件
	FILE *fp_lstb = fopen("listen_buff.txt","w+")
	int l = 0;
	while(listen_buffer[l] != '\0')
	{
		fprintf(fp_lstb, "%c", listen_buffer[l]);
		l++;
	}
	fclose(fp_lstb)
	//==================================================
